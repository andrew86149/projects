...
void centerOrigin(T& drawable)
{
	...
	drawable.setOrigin({bound.size.x/2, bound.size.y/2});
}
...
:GameState(game),m_text(game->getFont()),m_sprite(game->getLogo())
{
...
:GameState(game),m_playingState(playingState)
,m_text(game->getFont())
...
, m_pacWoman(nullptr),m_level(0),m_liveCount(3),m_score(0)
,m_liveSprite{sf::Sprite(game->getTexture()),sf::Sprite(game->getTexture()),sf::Sprite(game->getTexture())}
,m_scoreText(game->getFont()),m_levelText(game->getFont()),m_remainingDotsText(game->getFont())
{
...
:GameState(game),m_text(game->getFont())
,m_playingState(static_cast<PlayingState*>(playingState))
{
	//m_text.setFont(game->getFont());
...
:GameState(game),m_text(game->getFont()),m_countDownText(game->getFont())
,m_playingState(static_cast<PlayingState*>(playingState))
{
	//m_text.setFont(game->getFont());
...
{
	//m_playingState->update(delta); //Ошибка сегментирования (образ памяти сброшен на диск)
}
void GetReadyState::draw(sf::RenderWindow& window)
{
	//m_playingState->draw(window); //Ошибка сегментирования (образ памяти сброшен на диск)
	window.draw(m_text);
}
...
void PlayingState::update(sf::Time delta)
{
	...
	for (Ghost* ghost : m_ghosts)
	{
		if (ghost->getCollisionBox().findIntersection(m_pacWoman->getCollisionBox()))
		{
			if (ghost->isWeak())
			{
...
void PlayingState::updateCameraPosition()
{
	...
	if (m_camera.getCenter().y > m_maze.getSize().y *32 - 240)
		m_camera.setCenter({m_camera.getCenter().x, static_cast<float>(m_maze.getSize().y *32 - 240)});
}
