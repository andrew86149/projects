...
void Maze::loadLevel(std::string name)
{
	...
	if (!levelData.loadFromFile("assets/levels/" + name + ".png"))
        ...
        else
        {
		 ...
		 //m_renderTexture.resize({32* m_mazeSize.x, 32* m_mazeSize.y});
		 if(!m_renderTexture.resize({static_cast<unsigned int> (32* m_mazeSize.x), static_cast<unsigned int>(32* m_mazeSize.y)})) return;
		 ...
		 if (isWall(position))
		 {
		 	wall.setPosition({static_cast<float>(32*position.x), static_cast<float>(32*position.y)});
			...
			{
				border.setRotation(sf::degrees(0));
				m_renderTexture.draw(border);
			}
			
			if (!isWall(position + sf::Vector2i(0, 1)))
			{
				border.setRotation(sf::degrees(90));
				m_renderTexture.draw(border);
			}
			
			if (!isWall(position + sf::Vector2i(-1, 0)))
			{
				border.setRotation(sf::degrees(180));
				m_renderTexture.draw(border);
			}
			
			if (!isWall(position + sf::Vector2i(0, -1)))
			{
				border.setRotation(sf::degrees(270));
				m_renderTexture.draw(border);
			}
			
			if (isWall(position + sf::Vector2i(1, 0)) && isWall(position + sf::Vector2i(0, -1)))
			{
				innerCorner.setRotation(sf::degrees(0));
				m_renderTexture.draw(innerCorner);
			}
			
			if (isWall(position + sf::Vector2i(0, 1)) && isWall(position + sf::Vector2i(1, 0)))
			{
				innerCorner.setRotation(sf::degrees(90));
				m_renderTexture.draw(innerCorner);
			}
			
			if (isWall(position + sf::Vector2i(-1, 0)) && isWall(position + sf::Vector2i(0, 1)))
			{
				innerCorner.setRotation(sf::degrees(180));
				m_renderTexture.draw(innerCorner);
			}
			
			if (isWall(position + sf::Vector2i(0, -1)) && isWall(position + sf::Vector2i(-1, 0)))
			{
				innerCorner.setRotation(sf::degrees(270));
				m_renderTexture.draw(innerCorner);
			}
						
			if (!isWall(position + sf::Vector2i(1, 0)) && !isWall(position + sf::Vector2i(0, -1)))
			{
				outerCorner.setRotation(sf::degrees(0));
				m_renderTexture.draw(outerCorner);
			}
			
			if (!isWall(position + sf::Vector2i(0, 1)) && !isWall(position + sf::Vector2i(1, 0)))
			{
				outerCorner.setRotation(sf::degrees(90));
				m_renderTexture.draw(outerCorner);
			}
			
			if (!isWall(position + sf::Vector2i(-1, 0)) && !isWall(position + sf::Vector2i(0, 1)))
			{
				outerCorner.setRotation(sf::degrees(180));
				m_renderTexture.draw(outerCorner);
			}
			
			if (!isWall(position + sf::Vector2i(0, -1)) && !isWall(position + sf::Vector2i(-1, 0)))
			{
				outerCorner.setRotation(sf::degrees(270));
				m_renderTexture.draw(outerCorner);
			}			
		 }
		 }
...
void Maze::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	for (unsigned int i = 0; i < m_mazeData.size(); i++)
    {
        ...
        if (m_mazeData[i] == Dot)
        {
            dot.setPosition({static_cast<float>(32*position.x+16), static_cast<float>(32*position.y+16)});
            target.draw(dot, states);
        }
        else if (m_mazeData[i] == SuperDot)
        {
            superDot.setPosition({static_cast<float>(32*position.x+16), static_cast<float>(32*position.y+16)});
